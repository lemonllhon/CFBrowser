...
Trace Browser 当前推荐配套使用的浏览器内核，来源于开源项目 [fingerprint-chromium](https://github.com/adryfish/fingerprint-chromium)。

如果你正在寻找可直接下载和维护的指纹内核版本，建议先查看它的 Releases 页面：

- <https://github.com/adryfish/fingerprint-chromium/releases>

这个项目为 Trace Browser 的内核准备提供了直接可用的基础来源，这里先对原项目做明确推荐与致谢。

Trace Browser 的目标很明确：在一台桌面设备上，帮助用户稳定管理多个彼此隔离的浏览器实例，并配合代理池、浏览器内核和快捷启动能力完成日常运营或测试工作。
...
-- 浏览器重启功能-扩容功能 
###  2026-04-10
-- 指纹自动切换 地区国家切换 随机画像 内核管理
###  2026-03-19
-- 指纹自动切换
- 新增手动切换出口 置顶居中 代理池自动切换
...
2. 安装版直接运行 `TraceBrowser-Setup-*.exe`
3. 便携版解压后运行 `trace-browser.exe`
4. Linux 包下载后可直接安装 `trace-browser_<version>_<arch>.deb`，或解压 `tar.gz` 后运行 `trace-browser`
