...
var migrations = []migration{
	...
	},
	{
		...
		desc:    "实例表添加代理自动切换字段",
		stmts: []string{
			`ALTER TABLE browser_profiles ADD COLUMN auto_proxy_switch_enabled INTEGER NOT NULL DEFAULT 0`,
			`ALTER TABLE browser_profiles ADD COLUMN auto_proxy_switch_group_name TEXT NOT NULL DEFAULT ''`,
			`ALTER TABLE browser_profiles ADD COLUMN auto_proxy_switch_interval_m INTEGER NOT NULL DEFAULT 0`,
			`ALTER TABLE browser_profiles ADD COLUMN auto_proxy_switch_last_proxy_id TEXT NOT NULL DEFAULT ''`,
		},
	},
	{
		version: 8,
